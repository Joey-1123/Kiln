...
| M3-D1 | 2026-08-26 | VRAM preflight uses NF4-only formula: base_weights (0.5B/param) + LoRA (~10B/param) + activations; base gradients/optimizer omitted (frozen) | QLoRA freezes base — only LoRA params need grad + optimizer storage; conservatively estimates activation memory with 4× overhead factor | Accepted |
| M3-D2 | 2026-08-26 | config_sha always validates raw dicts through Pydantic before hashing | Ensures raw dict and validated KilnConfig produce identical hashes (defaults injected consistently); prevents silent fingerprint divergence | Accepted |
